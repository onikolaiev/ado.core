Describe "Get-ADOWitFieldList Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ADOWitFieldList).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Get-ADOWitFieldList).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-ADOWitFieldList).Parameters['Token']
		...
		It 'Should have the expected parameter Project' {
			$parameter = (Get-Command Get-ADOWitFieldList).Parameters['Project']
		...
		It 'Should have the expected parameter Expand' {
			$parameter = (Get-Command Get-ADOWitFieldList).Parameters['Expand']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Get-ADOWitFieldList).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-ADOWitFieldList).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -Token
		__AllParameterSets -Organization -Token -Project -Expand -ApiVersion -ProgressAction
		#>
	}
