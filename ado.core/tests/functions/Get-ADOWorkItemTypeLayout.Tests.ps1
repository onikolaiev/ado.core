Describe "Get-ADOWorkItemTypeLayout Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ADOWorkItemTypeLayout).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Get-ADOWorkItemTypeLayout).Parameters['Organization']
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-ADOWorkItemTypeLayout).Parameters['Token']
		...
		It 'Should have the expected parameter ProcessId' {
			$parameter = (Get-Command Get-ADOWorkItemTypeLayout).Parameters['ProcessId']
		...
		It 'Should have the expected parameter WitRefName' {
			$parameter = (Get-Command Get-ADOWorkItemTypeLayout).Parameters['WitRefName']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Get-ADOWorkItemTypeLayout).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-ADOWorkItemTypeLayout).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -Token -ProcessId -WitRefName
		__AllParameterSets -Organization -Token -ProcessId -WitRefName -ApiVersion -ProgressAction
		#>
	}
