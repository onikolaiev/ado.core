Describe "Get-ADOTeam Unit Tests" -Tag "Unit" {
	BeforeAll {
	...
	Describe "Ensuring unchanged command signature" {
		It "should have the expected parameter sets" {
			(Get-Command Get-ADOTeam).ParameterSets.Name | Should -Be '__AllParameterSets'
		}
		
		It 'Should have the expected parameter Organization' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['Organization']
		...
		It 'Should have the expected parameter ProjectId' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['ProjectId']
		...
		It 'Should have the expected parameter TeamId' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['TeamId']
			$parameter.Name | Should -Be 'TeamId'
			$parameter.ParameterType.ToString() | Should -Be System.String
			$parameter.IsDynamic | Should -Be $False
			$parameter.ParameterSets.Keys | Should -Be '__AllParameterSets'
			$parameter.ParameterSets.Keys | Should -Contain '__AllParameterSets'
		...
		It 'Should have the expected parameter Token' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['Token']
		...
		It 'Should have the expected parameter ExpandIdentity' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['ExpandIdentity']
		...
		It 'Should have the expected parameter ApiVersion' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['ApiVersion']
		...
		It 'Should have the expected parameter ProgressAction' {
			$parameter = (Get-Command Get-ADOTeam).Parameters['ProgressAction']
	...
	Describe "Testing parameterset __AllParameterSets" {
		<#
		__AllParameterSets -Organization -ProjectId -TeamId -Token
		__AllParameterSets -Organization -ProjectId -TeamId -Token -ExpandIdentity -ApiVersion -ProgressAction
		#>
	}
